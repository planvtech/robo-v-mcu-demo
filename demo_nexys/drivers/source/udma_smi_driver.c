...
uint8_t phy_addr_const;

udma_smi_open(uint8_t phy_addr)
{
...
uint16_t udma_smi_read(uint8_t reg_addr)
{
	UdmaSmi_t*	psmi = (UdmaSmi_t*)(UDMA_CH_ADDR_SMI);
		...
		return (uint16_t)psmi->rx_data;
};
udma_smi_rst(uint8_t rst_val)
{
	UdmaSmi_t*	psmi = (UdmaSmi_t*)(UDMA_CH_ADDR_SMI);
	while(psmi->phy_busy == 1)
	{
